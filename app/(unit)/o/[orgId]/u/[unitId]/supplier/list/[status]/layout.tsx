  ...
  return (
    <>
      <div className="mb-3 flex flex-row justify-between">
        <Tabs defaultValue={statusParam}>
          <TabsList>
            {LIST.map((status) => {
              ...
              return (
                <Link
                  href={`/o/${orgId}/u/${unitId}/supplier/list/${status}`}
                  key={status}
                  passHref
                >
                  <TabsTrigger
        ...
        <Link href={`/o/${orgId}/u/${unitId}/supplier/create`} passHref>
          <Button size="sm">
            <PlusCircleIcon />
            <span className="sr-only sm:not-sr-only">{t("New supplier")}</span>
          </Button>
        </Link>
      </div>
      {props.children}
